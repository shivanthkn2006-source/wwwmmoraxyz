...
const HomePage = () => {
  ...
  const fetchGlobalPosts = async () => {
    ...
    try {
      ...
      const postsResult = await supabase
        .from('posts')
        .select('id, user_id, content, media_url, media_type, likes_count, comments_count, created_at, visibility')
      ...
      const userIds = [...new Set(postsResult.data.map(p => p.user_id))];
      ...
      const postsWithLikes = postsResult.data
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      setLoading(true);
      try {
        // ULTRA-OPTIMIZED: 2s timeout for critical path on low-end devices
        await settleWithin(fetchGlobalPosts(), 2000);
      } finally {
        setLoading(false);
      }
