...
import { generateImage as generateImageViaPollinationsService } from '@/services/pollinationsService';
...
const WebdropPage = () => {
  ...
  const { guard: guardResponse } = useNeuroSymbolicGuard('webdrop');
  ...
  const handleTextGeneration = async () => {
    ...
    try {
      ...
      const guardedText = guardResponse(data.text, { skipTruthEngine: true }).safeResponse;
      setGeneratedContent({ type: 'text', content: guardedText });
      updateUsage('text');
    } catch (error) {
      console.error('Text generation error:', error);
      toast.error('Failed to generate text');
  ...
  const handleImageGeneration = async () => {
    ...
    try {
      ...
      if (imageFiles.length > 0) {
        ...
        // Generate new image - Pollinations first, Gemini fallback
        try {
          const result = await generateImageViaPollinationsService(prompt);
          setGeneratedContent({ type: 'image', content: result.imageUrl });
          setImageCaption('');
          updateUsage('images');
          toast.success(`Image generated via ${result.provider}!`);
        } catch (imgError: any) {
          // Fallback to edge function directly
          const { data, error } = await supabase.functions.invoke('generate-image', {
            body: { prompt: prompt }
          });
