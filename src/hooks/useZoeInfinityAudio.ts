// ═══════════════════════════════════════════════════════════════════════════════
// ZOE INFINITY STANDALONE AUDIO SYSTEM
// Pure Zoe Infinity audio engine - no external platform dependencies
...
// Audio configuration for Zoe Infinity
const AUDIO_CONFIG = {
  ...
  // Zoe Infinity specific sounds
  orbPulse: { frequency: 520, duration: 0.12, type: 'sine' as OscillatorType },
  quantumSync: { frequency: 720, duration: 0.25, type: 'triangle' as OscillatorType },
  neuralActivate: { frequency: 380, duration: 0.18, type: 'sine' as OscillatorType },
};

export function useZoeInfinityAudio() {
  ...
  const getAudioContext = useCallback((): AudioContext | null => {
    ...
    try {
      ...
      console.warn('[ZoeInfinityAudio] AudioContext init failed:', err);
      return null;
    }
  }, []);
  ...
  const playTone = useCallback(
    (config: { frequency: number; duration: number; type: OscillatorType }) => {
      ...
      if (isSoundSuppressed()) {
        console.debug('[ZoeInfinityAudio] Sounds suppressed after platform purge');
        return;
      }
      ...
      try {
        ...
        console.warn('[ZoeInfinityAudio] Audio playback failed:', err);
      }
    },
    [getAudioContext]
  );
  ...
  const playBootChime = useCallback(() => {
    // Play a sequence of tones for boot chime - Zoe Infinity signature
  ...
  // Zoe Infinity specific sounds
  const playOrbPulse = useCallback(() => {
    playTone(AUDIO_CONFIG.orbPulse);
  }, [playTone]);

  const playQuantumSync = useCallback(() => {
    playTone(AUDIO_CONFIG.quantumSync);
  }, [playTone]);

  const playNeuralActivate = useCallback(() => {
    playTone(AUDIO_CONFIG.neuralActivate);
  }, [playTone]);
  ...
  const forceStopAllRingtones = useCallback(() => {
    console.log('[ZoeInfinityAudio] FORCE STOP: Killing all ringtone intervals');
  ...
  const playCallRingtone = useCallback((isIncoming: boolean = true) => {
    ...
    if (isRingtonePlayingRef.current) {
      console.log('[ZoeInfinityAudio] Ringtone already playing, skipping');
      return;
    }
    ...
    console.log(`[ZoeInfinityAudio] Starting ${isIncoming ? 'incoming' : 'outgoing'} ringtone`);
    ...
    const playRingSequence = () => {
      ...
      if (ringtoneInstanceRef.current !== currentInstance) {
        console.log('[ZoeInfinityAudio] Stale ringtone instance detected, stopping');
        return;
      }
  ...
  const stopCallRingtone = useCallback(() => {
    console.log('[ZoeInfinityAudio] stopCallRingtone called');
    forceStopAllRingtones();
  }, [forceStopAllRingtones]);
