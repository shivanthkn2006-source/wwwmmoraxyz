    ...
    >
      <div className="max-w-2xl mx-auto space-y-6">
        {messages.map((message) => {
          ...
          const lastUserText = message.role === 'assistant'
            ? messages.slice(0, messages.indexOf(message)).reverse().find(m => m.role === 'user')?.content
            : undefined;
          
          return (
            <div
              key={message.id}
              className={`flex animate-fade-in ${message.role === 'user' ? 'flex-col items-end' : 'flex-row items-start gap-3'}`}
            >
              {/* Real human avatar for assistant messages */}
              {message.role === 'assistant' && (
                <div className="flex-shrink-0 mt-1">
                  <ZoeAvatarEmotions chatText={lastUserText} size="sm" />
                </div>
              )}
              ...
              <div className="flex flex-col flex-1 min-w-0">
                ...
                {/* Text content */}
                <div
                  className={`max-w-[85%] text-base leading-relaxed ${message.role === 'user' ? 'text-muted-foreground' : 'text-foreground'}`}
                  style={{
                    textShadow: message.role === 'assistant' ? '0 0 20px rgba(255,255,255,0.1)' : 'none',
                  }}
                >
                  {hasCitations 
                    ? parseContentWithCitations(stripSSMLTags(message.content), (id) => handleCitationClick(message.id, id))
                ...
                {message.image && (
                  <div className="mt-4 relative group">
                    <img 
                      ...
                      className="rounded-xl max-w-[300px] shadow-lg border border-border"
                      style={{ boxShadow: '0 0 30px rgba(0,255,255,0.2)' }}
                    />
                    {message.image.caption && (
                      <div className="mt-2 text-sm text-muted-foreground italic">"{message.image.caption}"</div>
                    )}
                    {message.image.style && (
                      <div className="absolute top-2 right-2 text-xs bg-background/50 px-2 py-1 rounded text-primary">
                        {message.image.style}
                      </div>
                    )}
                  </div>
        ...
        {/* Typing indicator */}
        {isTyping && (
          <div className="flex items-start animate-fade-in gap-3">
            <div className="flex-shrink-0 mt-1">
              <ZoeAvatarEmotions chatText="" size="sm" />
            </div>
