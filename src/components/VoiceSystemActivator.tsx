...
export const VoiceSystemActivator: React.FC<VoiceSystemActivatorProps> = ({ onActivated }) => {
  ...
  // Listen for manual activation requests (e.g., autoplay/audio unlock blocked)
  useEffect(() => {
    const handleManualActivation = () => {
      if (onAuthPage) return;

      // IMPORTANT: even if activation was previously completed, browsers can re-block
      // autoplay/audio context after tab/app lifecycle changes. Always allow re-activation.
      setShowPrompt(true);
    };
  ...
  // Keep component mounted so it can re-open the dialog when audio gets blocked again.
      ...
      <DialogContent className="sm:max-w-md bg-background/95 backdrop-blur-xl border-primary/20">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            ...
            {activationComplete ? 'Re-enable Voice' : 'Enable Hands-Free Voice'}
          </DialogTitle>
          ...
          >
            {isActivating ? (
              ...
              <>
                ...
                {activationComplete ? 'Re-enable Voice' : 'Enable Voice'}
              </>
            )}
          </Button>
        </DialogFooter>
