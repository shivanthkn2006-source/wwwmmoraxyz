/**
 * ═══════════════════════════════════════════════════════════════════════════
 * DAY-LORD TELEMETRY — deterministic (no model calls, no hallucination)
 * Vedic weekday rulership used to colour Zoe's tone and the DHF archetype tag.
 * Mirrored on the client at src/lib/dayLord.ts — keep both in sync.
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
/** Day lord for a given instant, resolved in the user's IANA timezone. */
export function getDailyArchetype(date: Date = new Date(), timeZone = 'Asia/Kolkata'): DayLordTelemetry {
  let index = date.getUTCDay();
  try {
    ...
    /* invalid timezone → fall back to UTC weekday */
  }
